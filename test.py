# -*- coding: utf-8 -*-
"""
Created on Sun Mar 24 18:36:50 2024
...
# Read input images  
print(os.listdir("E:/fffiiles/college pracs and projects/Project/Car")) 
SIZE = 224 
#Channel=3  #Resize images 
#lists 
train_images = [] 
train_labels = []  
for directory_path in glob.glob("E:/fffiiles/college pracs and projects/Project/Car/Model_images/train/*"): label = directory_path.split("\\")[-1] 
print(label) 
for img_path in glob.glob(os.path.join(directory_path, "*.png")): print(img_path) 
...
test_images = [] 
test_labels = []  
for directory_path in glob.glob("E:/fffiiles/college pracs and projects/Project/Car/Model_images/test/*"): fruit_label = directory_path.split("\\")[-1] 
for img_path in glob.glob(os.path.join(directory_path, "*.png")): img = cv2.imread(img_path, cv2.IMREAD_COLOR) 
img = cv2.resize(img, (SIZE, SIZE)) 
img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR) 
...
#Split data  
x_train, y_train, x_test, y_test = train_images, 
train_labels_encoded, test_images, test_labels_encoded 
...
cm = confusion_matrix(test_labels, prediction_Logreg) 
#print(cm) 
